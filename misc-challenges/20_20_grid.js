...
// will check every array in matrix
for(let i = 0; i < m.length; i++){
    // will check every 4 digits in array horizontally
    for(let j = 0; j < m.length; j++){
        ...
        // diagonal down
        if(i<17 && j<17){
            let diagDown = m[i][j] * m[i + 1][i + 1] * m[i + 2][i + 2] * m[i + 3][i + 3]
        ...
        // diagonal up
        if (i < 17 && j > 2) {
            let diagUp = m[i][j] * m[i + 1][j - 1] * m[i + 2][j - 2] * m[i + 3][j - 3]
